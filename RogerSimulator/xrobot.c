...
void draw_streamlines()
{
  ...
  int i, row, col, bin_x, bin_y, already_used[NBINS][NBINS], count;
  ...
  for (row=1;row<(NBINS-1); ++row) {
	...
	count = 0;
	while ((Roger.world_map.occupancy_map[row][col] != GOAL) && 
	       (mag=compute_gradient(oldx, oldy, &Roger, grad)) &&
	       (count++ < 500)) {
	  
	  // if (col==1) {
	  //   printf("following %d %d: %d %d    %6.4lf %6.4lf   %d  %6.4lf\n",
	  //		   col, row, bin_x, bin_y, oldx, oldy,
	  //		   Roger.world_map.occupancy_map[bin_y][bin_x], mag);
	  // }
	  
	  x = oldx - STEP*grad[X];
	  y = oldy - STEP*grad[Y];

	  //	  bin_x = (int)((x-MIN_X)/XDELTA);
	  //	  bin_y = (int)((MAX_Y-y)/YDELTA);
